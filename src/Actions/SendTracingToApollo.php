...
class SendTracingToApollo
{
    ...
    private function serializeProtobufWithProtoc(Report $report): string
    {
        ...
        $process = proc_open(
            'protoc --encode=mdg.engine.proto.Report --proto_path=' . $protoPath . ' ' . $protoFile,
            [
                0 => ['pipe', 'rb'],
                1 => ['pipe', 'wb'],
                2 => ['pipe', 'wb'],
        ...
        if ($return !== 0) {
            throw new LogicException('protoc returned error code ' . $return . ':' . $err);
        }
     ...
     */
    private function writeProtobufValue($process, $value, int $depth = 0, bool $startInline = false): void
    {
        if (is_bool($value)) {
            fwrite($process, ($startInline ? '' : str_repeat(' ', $depth)) . $value ? 'true' : 'false');
        } elseif (is_float($value) || is_int($value)) {
            fwrite($process, ($startInline ? '' : str_repeat(' ', $depth)) . $value);
        } elseif (is_string($value)) {
            fwrite($process, ($startInline ? '' : str_repeat(' ', $depth)) . json_encode($value, JSON_THROW_ON_ERROR | JSON_UNESCAPED_SLASHES));
        } elseif ($value instanceof RepeatedField) {
            fwrite($process, ($startInline ? '' : str_repeat(' ', $depth)) . '[' . PHP_EOL);
            $count = count($value);
            ...
            foreach ($this->getMessageProperties($value) as $name => $getter) {
                ...
                if (
                    $propertyValue === null ||
                    $propertyValue === '' ||
                    ($propertyValue instanceof RepeatedField && count($propertyValue) === 0)
                ) {
                    continue;
                }

                fwrite($process, str_repeat(' ', $depth) . $name . ': ');
                $this->writeProtobufValue($process, $propertyValue, $depth, true);
            ...
            foreach ($value as $key => $child) {
                fwrite($process, ($first ? '' : ',' . PHP_EOL) . str_repeat(' ', $depth) . '{' . PHP_EOL);
                $first = false;
